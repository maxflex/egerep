    ...
    <div class="col-sm-12">
        ...
        <div>
            ...
            <div ng-show='tutor.statistics.hold_coeff_by_grade[11] > 0'>Коэффициент удержания для 11 класса: @{{ tutor.statistics.hold_coeff_by_grade[11] }}%  из @{{ tutor.statistics.ec_lesson_count }} <ng-pluralize count='tutor.statistics.ec_lesson_count' when="{
                ...
                'few': 'занятий',
                'many': 'занятий'
            }"></ng-pluralize> (@{{ tutor.statistics.coeff_total[11] }}%)</div>
            <div ng-show='tutor.statistics.hold_coeff > 0'>Общий коэффициент удержания: @{{ tutor.statistics.hold_coeff }}%  из @{{ tutor.statistics.ec_lesson_count }} <ng-pluralize count='tutor.statistics.ec_lesson_count' when="{
                'one': 'занятия',
                'few': 'занятий',
                'many': 'занятий'
            }"></ng-pluralize> (@{{ tutor.statistics.coeff_total[0] }}%)</div>
        </div>
        <div ng-if="tutor.id">
            <h4>КОММЕНТАРИИ</h4>
