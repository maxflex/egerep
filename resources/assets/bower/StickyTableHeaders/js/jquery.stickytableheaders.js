			...
			if (base.$el) {
				base.$el.each(function () {
					...
					if (scrolledPastTop && notScrolledPastBottom) {
						...
						if (pb_diff < 0) {
							new_options.clip = 'rect(0, ' + (base.panel_body_width + Math.abs(pb_diff) + 1) + 'px, 88px, ' + Math.abs(pb_diff) + 'px)'
						}
						if (pb_diff > 0) {
							new_options.clip = 'rect(0, ' + (base.panel_body_width - pb_diff + 1) + 'px, 88px, 0)'
						}
